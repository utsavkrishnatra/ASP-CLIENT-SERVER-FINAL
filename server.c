...
#include <dirent.h>
  ...
  //defining the port , buffer size and macro and mirror port
...
void search_directory(char* path, char* filename, char* command) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        // Checking if entry is a file or a directory
...
void processclient(int sockfd) {
    ...
    while (1) {
        ...
        if (token == NULL) {
            ...
            // if ( strcmp(args_arr[numExtensions - 1], "-u") == 0 && numExtensions >= 2) {
            //     unzip = 1;
            ...
            // if (numExtensions == 0 || numExtensions > 4) {
            //     printf("Extension list must have 1 to 4 extensions.\n");
            //     return 1;
            // }
